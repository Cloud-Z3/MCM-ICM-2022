...
def considerinmatch(matchid):
    ...
    for i in range(1,9):
        ...
        for j in ans1:
            ...
            line[2] = adj[a][b]
            matrix.append(line)
    df2=pd.DataFrame(matrix,columns=['TimeSpan','Dyadic Configuration','Count'])
    df2.to_csv(file2+'/'+p1+'/Dyadic Configuration(in match'+str(matchid)+').csv',index=False)
    plt.figure(dpi=600,figsize=(8,5))
    sns.lineplot(data=df2,x='TimeSpan',y='Count',hue='Dyadic Configuration')
    plt.savefig(file3+'/p1/matchi'+'/Dyadic Configuration(in match'+str(matchid)+').png')
    ...
    for i in range(1,9):
        ...
        for j in ans2:
            ...
            line[2] = adj[a][b]
            matrix.append(line)
    df2=pd.DataFrame(matrix,columns=['TimeSpan','Triadic Configuration','Count'])
    df2.to_csv(file2+'/'+p1+'/Triadic Configuration(in match'+str(matchid)+').csv',index=False)
    plt.figure(dpi=600,figsize=(8,5))
    sns.lineplot(data=df2,x='TimeSpan',y='Count',hue='Triadic Configuration')
    plt.savefig(file3+'/p1/matchi'+'/Triadic Configuration(in match'+str(matchid)+').png')
