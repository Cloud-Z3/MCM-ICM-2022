# Find the 2_nd smallest eigenvalue.
...
def considerinmatch(matchid):
    ...
    for i in range(1,9):
        ...
        eigenvalue=list(([abs(j) for j in list(eigenvalue)]))
        eigenvalue.sort()
        print(eigenvalue)
        line[1] = eigenvalue[1]
        matrix.append(line)
    df = pd.DataFrame(matrix, columns=['TimeSpan', 'Connectivity'])
    df.to_csv(file2 + '/' + p1 + '/Connectivity'+str(matchid)+'.csv')
    plt.figure(dpi=600, figsize=(8, 5))
    sns.lineplot(data=df, x='TimeSpan', y='Connectivity', color='g')
    plt.savefig(file3 + '/p1' + '/Connectivity'+str(matchid)+'.png')
...
def considerallmatch():
    ...
    for i in range(1,39):
        ...
        line[1] = eigenvalue[1]
        matrix.append(line)
    df=pd.DataFrame(matrix,columns=['MatchID','Connectivity'])
    df.to_csv(file2+'/'+p1+'/Connectivity.csv')
    plt.figure(dpi=600, figsize=(8, 5))
    sns.lineplot(data=df, x='MatchID', y='Connectivity',color='g')
    plt.savefig(file3 + '/p1' + '/Connectivity.png')

def considerallmatchaverage():
    matrix=[[i,0] for i in range(1,9)]#TimeSpan, average_max_eigen
    for i in range(1,39):
        df=pd.read_csv(file2 + '/' + p1 + '/Connectivity'+str(i)+'.csv')
        for j in df.index:
            matrix[j][1]+=df['Connectivity'][j]
    for i in range(8):
        matrix[i][1]/=39
    df=pd.DataFrame(matrix,columns=['TimeSpan','Connectivity'])
    df.to_csv(file2 + '/' + p1 + '/Connectivity_ave.csv')
    plt.figure(dpi=600, figsize=(8, 5))
    sns.lineplot(data=df, x='TimeSpan', y='Connectivity', color='g')
    plt.savefig(file3 + '/p1' + '/Connectivity_ave.png')
